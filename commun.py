...
def choose_your_player_func():
    ...
    draw_element(choose_car, screen_placement(WINDOW_LENGTH, 115, 0),5)
    delay(1000)
    while(not player_ok):

      blink_element(WINDOW_LENGTH//2, WINDOW_HEIGHT//2+15)
      
      choose_plr = lis3dsh_driver.get_acc_value()
      print("chosse plr : ", choose_plr)
      
      if choose_plr < -300:
          blink_element(screen_placement(WINDOW_LENGTH, 10, 1), (WINDOW_HEIGHT//2) +15)
          player_ok = True
          player_choose = birdython
      elif choose_plr > 300:
          blink_element((WINDOW_LENGTH//2)+screen_placement(WINDOW_LENGTH, 10, 1), (WINDOW_HEIGHT//2) +15)
          player_ok = True
          player_choose = thon
    
    if player_choose == birdython:
      blink_element(screen_placement(WINDOW_LENGTH, 10, 1), (WINDOW_HEIGHT//2) +15)
    else:
      blink_element((WINDOW_LENGTH//2)+screen_placement(WINDOW_LENGTH, 10, 1), (WINDOW_HEIGHT//2) +15)
